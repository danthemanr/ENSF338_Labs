...
def my_funct(x, quad, loglinear, linear, log, const):
    if type(x) != int and type(x) != float:
        return_y = [(quad*i*i + linear*i + loglinear*i*(math.log(i, 2)) + log*(math.log(i, 2)) + const) for i in x]
    else:
        return_y = (quad*x*x + linear*x + loglinear*x*(math.log(x, 2)) + log*(math.log(x, 2)) + const)
    return return_y

def main(plotnum=""):
    sizes = [10, 20, 50, 100, 200, 500, 1000, 2000, 5000]
    pts = len(sizes)
    vectors = make_vectors(sizes, sorted=True) #makes quicksort worse by sorting the vectors
    linear_times = []
    binary_times = []
    i = 0
    #the loop takes ~20 minutes to run
    while  i < pts:
        vector = vectors[i]
        end = sizes[i]-1
    ...
    a, loglin, m, log, b = scipy.optimize.curve_fit(my_funct, sizes, binary_times)[0]
    print(f"linear equation: {slope}(n) + {intercept}")
    print(f"binary equation: ({a})n^2 + ({loglin})nlog(n) + ({m})n + ({log})log(n) + {b}")

    fig, axs = plt.subplots(1, 2, figsize=(10, 5))
    axs[0].scatter(sizes, linear_times, alpha=0.5)
    axs[0].plot(sizes, [slope*x+intercept for x in sizes], label="linear")
    ...
    axs[1].plot(sizes, my_funct(sizes, a, loglin, m, log, b), label="binary")
    axs[1].set_title("binary_search")
    axs[1].set_xlabel("vector size")
    axs[1].set_ylabel("seconds for 100 random tasks")
    #axs[0].legend()
    #plt.show()
    plt.savefig(f"./images/output6{plotnum}_worst.png")
...
# # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # 
# 4. Binary Search with worse case quicksort is much slower than linear sort.
#
